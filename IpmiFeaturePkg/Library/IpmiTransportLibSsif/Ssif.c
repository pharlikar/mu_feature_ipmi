  ...
  while (!Done) {
    ...
    if (SMBusCmd == SMBUS_CMD_READ) {
      ...
      if (BlockBuffer[0] == 0xFF) {
        ...
        if ((BlockBuffer[0] != BlockNumber) && (BlockBuffer[0] != BlockNumber + 1)) {
          DEBUG ((DEBUG_ERROR, "[SSIF] SMBus block out of order! Expected (%d or %d), Received %d.\n", BlockNumber, BlockNumber + 1, BlockBuffer[0]));
